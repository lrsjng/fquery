...
module.exports = function (fQuery) {
	...
	fQuery.rmfr = function (iamsure, filepath) {
		...
		if (iamsure !== I_AM_SURE) {
			fQuery.error({ method: 'rmfr', message: 'first argument needs to be fQuery.I_AM_SURE', data: Array.prototype.slice.call(arguments)});
		}
	...
	var plugin = {
		...
		remove: function (iamsure) {
			...
			if (iamsure !== I_AM_SURE) {
				fQuery.error({ method: 'remove', message: 'first argument needs to be fQuery.I_AM_SURE', fquery: this});
			}
