...
var _ = require('underscore'),
	...
	formatLine = function (content, len, idx) {

		var s = '';

		if (_.isNumber(idx)) {
			s += color('cyan', align(idx, 3, true, 0));
			s += '  ';
		}
	...
	formatLines = function (content, len, from, to) {
		...
		var s = '';
		for (var i = from - 1, l = lines.length; i < to && i < l; i += 1) {
			s += formatLine(lines[i], len, i + 1);
		}
	...
	formatHeader = function (idx, source, timestamp) {
		...
		s += '  ' + color('yellowL', timestamp.format());
		return s;
	},
	...
	formatBlob = function (blob, lines, len, idx) {
		...
		var s = formatHeader(idx, blob.source, blob.timestamp);
		s += lines ? '\n' : '  ';
		...
		if (blob.content === undefined && blob.buffer === undefined) {
