/*jshint node: true, strict: false */

var path = require('path'),
	_ = require('underscore'),

	Event = require('./Event'),
	Blob = require('./Blob'),
	Selector = require('./Selector'),

	selector = new Selector({ files: true, dirs: false, uniq: true, onlyStats: false }),

	publishTop = function (fquery) {

		var top = fquery._stack[fquery._stack.length - 1],
			i, l;

		for (i = 0, l = top.length; i < l; i += 1) {
			fquery[i] = top[i];
		}
		for (i = top.length; fquery[i] !== undefined; i += 1) {
			delete fquery[i];
		}

		fquery.length = top.length;
	},

	FQuery = function () {};



// fQuery
// ======
var fQuery = module.exports = function () {

	var fquery = new FQuery();
	fquery._stack = [];
	return fquery.pushStack(selector.blobs(Array.prototype.slice.call(arguments)));
};

fQuery.fn = fQuery.prototype = FQuery.prototype = {};

fQuery.extend = function (obj) {

	_.extend(fQuery, obj);
};

fQuery.fn.extend = function (obj) {

	_.extend(fQuery.fn, obj);
};



fQuery.extend({

	error: function (method, message, fquery, blob, err, line, column) {

		Event.error(method, message, fquery, blob, line, column, err);
	},

	plugin: function (plugin) {

		if (_.isString(plugin)) {
			try {
				plugin = require(plugin);
			} catch (err) {
				fQuery.error('plugin', err.toString(), undefined, undefined, err);
			}
		}
		if (_.isFunction(plugin)) {
			plugin = plugin(fQuery);
		}
		if (_.isObject(plugin)) {
			fQuery.fn.extend(plugin);
		} else {
			fQuery.error('plugin', 'unsupported format: ' + plugin, undefined, undefined, plugin);
		}
	},

	virtualBlob: Blob.virtual,

	selectBlob: Blob.select,

	event: Event
});




// fQuery Core
// -----------
fQuery.fn.extend({

	constructor: fQuery,

	error: function (method, message, blob, err, line, column) {

		Event.error(method, message, this, blob, line, column, err);
		return this;
	},

	pushStack: function (blobs) {

		blobs = _.filter(_.isArray(blobs) ? blobs : [blobs], function (blob) {

			return blob instanceof Blob;
		});

		this._stack.push(blobs);
		publishTop(this);
		return this;
	},

	end: function () {

		this._stack.pop();
		if (!this._stack.length) {
			this._stack.push([]);
		}
		publishTop(this);
		return this;
	},

	get: function (idx) {

		if (!_.isNumber(idx)) {
			return Array.prototype.slice.call(this);
		}

		return idx < 0 ? this[this.length + idx] : this[idx];
	},

	each: function (fn) {

		_.each(this, function (blob, idx) {

			fn.call(blob, blob, idx);
		});

		return this;
	},

	edit: function (fn, all) {

		var list = [];

		_.each(this, function (blob, idx) {

			var clone = blob.clone();
			if (all || clone.content !== undefined) {
				fn.call(clone, clone, idx);
			}
			list.push(clone);
		});

		return this.pushStack(list);
	},

	toString: function (lines, len) {

		var s = '';

		for (var i = 0, l = this.length; i < l; i += 1) {
			s += this[i].toString(lines, len, i);
		}

		return s + '\n';
	},

	log: function (lines, len) {

		console.log(this.toString(lines, len));
		return this;
	}
});



// load plugins
// ------------
_.each(selector.paths(path.join(__dirname, 'fQuery/*.js'), { files: true, dirs: false, uniq: true, onlyStats: true }), function (filepath) {

	fQuery.plugin('./fQuery/' + path.basename(filepath, '.js'));
});

_.each(selector.paths(path.join(__dirname, '../plugins/*'), { files: false, dirs: true, uniq: true, onlyStats: true }), function (dirpath) {

	fQuery.plugin('../plugins/' + path.basename(dirpath));
});
