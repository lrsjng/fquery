...
module.exports = fQuery => {
    ...
    fQuery.fn.newerThan = function fn1(arg, deps, keepAll) {
        let keepThisBlobs = [];
        const depsStamps = fQuery(deps).map(blob => blob.timestamp);
        ...
        this.each(function fn2(blob, idx) {
            ...
            if (!targetBlob) {
                ...
                targetStamp = targetBlob.timestamp;

                if (blob.timestamp > targetStamp || depsNewerThanTarget(depsStamps, targetStamp)) {
                    keepThisBlobs.push(blob);
                }
            }
        });
