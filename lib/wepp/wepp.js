...
var path = require('path'),
	...
	processFile = function (options, inFile, outFile) {
		...
		if (!inFile || !outFile) {
			fQuery.Event.error({ method: 'processFile', message: 'input and output file must be specified' });
		}
		...
		if (ext === '.css' || ext === '.less') {
			...
			fQuery.Event.info({ method: 'css' + compressionMark, message: message });
			f.less();
			...
			fQuery.Event.info({ method: 'js' + compressionMark, message: message });
			f.includify();
			if (options.compression) {
				f.uglifyjs();
			}
			...
			fQuery.Event.error({ method: 'processFile', message: 'unsupported extension: ' + ext });
		}
		...
		if (options.size) {
			...
			fQuery.Event.info({ method: 'size', message: bytesDest + ' bytes (~' + bytesGzip + ' zipped)' });
		}
	},
	...
	processDir = function (options, inDir, outDir) {
		...
		if (!inDir || !outDir) {
			fQuery.Event.error({ method: 'processDir', message: 'input and output dir must be specified' });
		}

		fQuery.Event.info({ method: 'processDir', message: inDir + ' -> ' + outDir });
	...
	processOptions = function (options) {
		...
		try {
			...
			if (options.inFile && options.outFile) {
				...
				processFile(options, options.inFile, options.outFile);

			} else if (options.inDir && options.outDir) {

				options.inDir = path.resolve(options.inDir);
				options.outDir = path.resolve(options.outDir);
				processDir(options, options.inDir, options.outDir);

			} else {

				fQuery.Event.error({ method: 'wepp', message: 'either input and output file or input and output directory must be specified' });
