...
fQuery.fn.extend({
	...
	push_instant: function (blobs) {
		...
		blobs = blobs.filter(function (blob) { return blob instanceof Blob; });
...
var paths = [];
paths = paths.concat(selector.paths(path.resolve(__dirname, 'core/*.js'), { files: true, dirs: false, uniq: true, onlyStats: true }));
paths = paths.concat(selector.paths(path.resolve(__dirname, '../plugins/*'), { files: false, dirs: true, uniq: true, onlyStats: true }));
paths.forEach(function (path) { fQuery.plugin(path); });
