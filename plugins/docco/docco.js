var MODULE_PATH = __dirname + '/../../node_modules/docco';
...
module.exports = function(OPTIONS) {
  ...
  fs = require('fs');
  path = require('path');
  showdown = require(MODULE_PATH + '/vendor/showdown').Showdown;
  ...
  destination = function(filepath) {
    return path.join(OPTIONS.dest, path.basename(filepath, path.extname(filepath)) + '.html');
  };
  ensure_directory = function(callback) {
    return exec('mkdir -p "' + OPTIONS.dest + '"', function() {
      return callback();
    });
  };
  ...
  docco_template = template(fs.readFileSync(MODULE_PATH + '/resources/docco.jst').toString());
  docco_styles = fs.readFileSync(MODULE_PATH + '/resources/docco.css').toString();
  highlight_start = '<div class="highlight"><pre>';
  highlight_end = '</pre></div>';
  sources = OPTIONS.files.sort();
  if (sources.length) {
    ensure_directory(function() {
      var files, next_file;
      fs.writeFile(path.join(OPTIONS.dest, 'docco.css'), docco_styles);
